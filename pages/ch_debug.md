...
-   ***Critical*** -- set severity level to **`eDiag_Critical`**
...
The ***CException*** class has numerous reporting methods (the contents of reports is defined by [diagnostics post flags](ch_core.html#ch_core.diag_post_flags)):
...
There are a [number of ways](#ch_debug.reporting_exceptions) to report ***CException***, for example:
...
The `TBase` template parameter is an exception base class such as ***CException*** or ***CCoreException***, or another class similar to these. The ***CStrErrAdapt::strerror*** template parameter is a function defined in an adaptor class for getting the error description string. The ***CErrnoTemplException*** has only one [error core](#ch_debug.excep_err_codes) - **`eErrno`** defined in the parent class, ***ErrnoTemplExceptionEx***. To analyze the actual reason of the exception one should use ***GetErrno()*** method:
